<?php
	...
	if($dept=='All')
	{
		...
		$r=mysqli_query($con,"select id,app_date,app_time,patients.name,phno,doc_name,departments.name,status from patients inner join departments on department=dept_id order by app_date desc,app_time desc");
			...
			$r=mysqli_query($con,"select id,app_date,app_time,patients.name,phno,doc_name,departments.name,status from patients inner join departments on department=dept_id where doc_name='$doctor' order by app_date desc,app_time desc");
	}
	else
	{
		if($doctor=="All")
